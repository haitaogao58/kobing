...
mod tests {
    ...
    #[test]
    fn backend_config_accepts_injector_and_legacy_ko_bing_alias() {
        let injector: MainConfig = toml::from_str(r#"backend = "injector""#).unwrap();
        assert_eq!(injector.backend, Backend::Injector);

        let ko_bing: MainConfig = toml::from_str(r#"backend = "ko_bing""#).unwrap();
        assert_eq!(ko_bing.backend, Backend::Injector);

        assert!(toml::from_str::<MainConfig>(r#"backend = "ts""#).is_err());
        assert!(toml::from_str::<MainConfig>(r#"backend = "Injector""#).is_err());
        assert!(toml::from_str::<MainConfig>(r#"backend = "KOBING""#).is_err());
