...
impl ECDHPrivateKey {
    ...
    pub(crate) fn decrypt_message_ko_bing_legacy(
        &self,
        sender_public_key: &[u8],
        salt: &[u8],
        iv: &[u8],
        ...
        let hkdf = ko_bing_legacy_kdf_extract(sender_public_key, salt)
            .context(err!("legacy extract on sender_public_key failed"))?;
        let hkdf = ko_bing_legacy_kdf_extract(&recipient_public_key, &hkdf)
        ...
        .context(err!("ecdh_compute_key failed"))?;
        let prk = ko_bing_legacy_kdf_extract(&secret, &hkdf)
            .context(err!("legacy extract on secret failed"))?;
        let aes_key = ko_bing_legacy_kdf_expand(AES_256_KEY_LENGTH, &prk, b"AES-256-GCM key")
...
mod test {
    ...
    #[test]
    fn test_ko_bing_legacy_message_can_be_decrypted() -> Result<()> {
        let recipient_private_key = [
            48, 71, 2, 1, 1, 4, 66, 0, 216, 94, 142, 86, 101, 129, 151, 127, 114, 240, 45, 28, 56,
            43, 44, 252, 219, 19, 57, 133, 209, 161, 60, 194, 143, 94, 110, 26, 106, 99, 103, 49,
            131, 222, 230, 146, 210, 82, 56, 123, 56, 210, 22, 104, 232, 251, 30, 109, 73, 205,
        ...
        // Fixed output from the accidental PBKDF2-based HKDF adapter shipped by older KOBING builds.
        ...
        let message = recipient.decrypt_message_ko_bing_legacy(
            &sender_public_key,
            &salt,
            &iv,
            &ciphertext,
