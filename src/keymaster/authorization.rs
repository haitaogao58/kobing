...
use crate::keymaster::keymint_device::{localize_auth_token_for_ko_bing, KeyMintDevice};
...
use crate::top::kobing::ko_bing::IKoBingAuthorizationService::{
    BnKoBingAuthorizationService, IKoBingAuthorizationService,
};
use crate::{
    config::config,
...
impl AuthorizationManager {
    ...
    pub fn new_ko_bing_binder() -> Result<Strong<dyn IKoBingAuthorizationService>> {
        let mgr = Self::new_manager();
        Ok(BnKoBingAuthorizationService::new_binder_with_features(
            mgr,
            crate::consts::sid_features(),
        ))
    }
    ...
    fn add_auth_token(
        ...
        if let Some(localized) =
            localize_auth_token_for_source(ctx, auth_token, localize_auth_token_for_ko_bing)?
        {
...
fn check_ko_bing_keystore_permission(
    ctx: &CallerInfo,
    perm: KeystorePerm,
    label: &str,
) -> BinderResult<()> {
...
fn localize_auth_token_for_source(
    ...
    match localize(auth_token).context(ks_err!("localizing trusted auth token for KOBING")) {
        Ok(localized) => Ok(Some(localized)),
        Err(error) if ctx.is_some() => {
            log::warn!(
                "dropping auth token mirrored after System success because KOBING localization failed authType={:#x} challengeTag={:04x}: {error:#}",
                auth_token.authenticatorType.0,
                challenge_tag(auth_token.challenge),
            );
            Ok(None)
...
impl IKoBingAuthorizationService for AuthorizationManager {
    fn addAuthToken(
        &self,
        ctx: Option<&CallerInfo>,
        auth_token: &HardwareAuthToken,
    ) -> BinderResult<()> {
        let _wp = wd::watch("IKoBingAuthorizationService::addAuthToken");
        let ctx = require_ko_bing_ctx(ctx, "IKoBingAuthorizationService::addAuthToken")?;
        check_ko_bing_keystore_permission(
            ctx,
    ...
    fn onDeviceUnlocked(
        ...
        let _wp = wd::watch("IKoBingAuthorizationService::onDeviceUnlocked");
        let ctx = require_ko_bing_ctx(ctx, "IKoBingAuthorizationService::onDeviceUnlocked")?;
        check_ko_bing_keystore_permission(
            ctx,
            KeystorePerm::Unlock,
    ...
    fn onDeviceLocked(
        ...
        let _wp = wd::watch("IKoBingAuthorizationService::onDeviceLocked");
        let ctx = require_ko_bing_ctx(ctx, "IKoBingAuthorizationService::onDeviceLocked")?;
        check_ko_bing_keystore_permission(
            ctx,
            KeystorePerm::Lock,
    ...
    fn onUserStorageLocked(&self, ctx: Option<&CallerInfo>, user_id: i32) -> BinderResult<()> {
        let _wp = wd::watch("IKoBingAuthorizationService::onUserStorageLocked");
        let ctx = require_ko_bing_ctx(ctx, "IKoBingAuthorizationService::onUserStorageLocked")?;
        check_ko_bing_keystore_permission(
            ctx,
            KeystorePerm::Lock,
            "caller missing Lock permission",
    ...
    fn onWeakUnlockMethodsExpired(
        ...
        let _wp = wd::watch("IKoBingAuthorizationService::onWeakUnlockMethodsExpired");
        let ctx = require_ko_bing_ctx(
            ctx,
            "IKoBingAuthorizationService::onWeakUnlockMethodsExpired",
        )?;
        check_ko_bing_keystore_permission(
            ctx,
            KeystorePerm::Lock,
    ...
    fn onNonLskfUnlockMethodsExpired(
        ...
        let _wp = wd::watch("IKoBingAuthorizationService::onNonLskfUnlockMethodsExpired");
        let ctx = require_ko_bing_ctx(
            ctx,
            "IKoBingAuthorizationService::onNonLskfUnlockMethodsExpired",
        )?;
        check_ko_bing_keystore_permission(
            ctx,
            KeystorePerm::Lock,
    ...
    fn getAuthTokensForCredStore(
        ...
        let _wp = wd::watch("IKoBingAuthorizationService::getAuthTokensForCredStore");
        let ctx = require_ko_bing_ctx(
            ctx,
            "IKoBingAuthorizationService::getAuthTokensForCredStore",
        )?;
        check_ko_bing_keystore_permission(
            ctx,
    ...
    fn getLastAuthTime(
        ...
        let _wp = wd::watch("IKoBingAuthorizationService::getLastAuthTime");
        let ctx = require_ko_bing_ctx(ctx, "IKoBingAuthorizationService::getLastAuthTime")?;
        check_ko_bing_keystore_permission(
            ctx,
