...
use crate::keymaster::permission::{
    ...
    check_unique_id_attestation_permissions, require_ko_bing_ctx, KeyPerm,
};
...
use crate::top::kobing::ko_bing::{
    ...
    IKoBingSecurityLevel::{BnKoBingSecurityLevel, IKoBingSecurityLevel},
};
...
struct KoBingSecurityLevelWrapper {
    inner: Arc<KeystoreSecurityLevel>,
}
...
impl Deref for KoBingSecurityLevelWrapper {
    type Target = KeystoreSecurityLevel;
...
impl KeystoreSecurityLevel {
    ...
    pub fn new_binders(
        ...
        Strong<dyn IKoBingSecurityLevel>,
    )> {
        ...
        let ko_bing = BnKoBingSecurityLevel::new_binder_with_features(
            KoBingSecurityLevelWrapper { inner },
            crate::consts::sid_features(),
        );
        Ok((aosp, ko_bing))
    }
    ...
    fn create_operation(
        ...
        let km_blob = SUPER_KEY
            .read()
            .unwrap()
            .unwrap_key_if_required_with_ko_bing_compatibility(&blob_metadata, km_blob)
            .context(ks_err!("Failed to handle super encryption."))?;
    ...
    fn import_key(
        ...
        let creation_result = map_km_error({
            ...
            km_dev.importKey(&params, format, key_data, None)
        })
    ...
    fn import_wrapped_key(
        ...
        let wrapping_key_blob = SUPER_KEY
            .read()
            .unwrap()
            .unwrap_key_if_required_with_ko_bing_compatibility(
...
impl binder::Interface for KoBingSecurityLevelWrapper {}
...
impl IKoBingSecurityLevel for KoBingSecurityLevelWrapper {
    fn createOperation(
        &self,
        ctx: Option<&CallerInfo>,
        key: &KeyDescriptor,
        ...
        let ctx = Some(require_ko_bing_ctx(
            ctx,
            "IKoBingSecurityLevel::createOperation",
        )?);
        let _wp = self.watch("IKoBingSecurityLevel::createOperation");
        security_level_manager::notify_operation_performed(self.security_level);
        let (latency, result) =
    ...
    fn generateKey(
        ...
        let ctx = Some(require_ko_bing_ctx(
            ctx,
            "IKoBingSecurityLevel::generateKey",
        )?);
        let _wp = self.watch_millis("IKoBingSecurityLevel::generateKey", 5000);
        security_level_manager::notify_operation_performed(self.security_level);
        let (latency, result) = crate::timed_call!(self.generate_key(
            ctx,
            key,
    ...
    fn importKey(
        ...
        let ctx = Some(require_ko_bing_ctx(ctx, "IKoBingSecurityLevel::importKey")?);
        let _wp = self.watch("IKoBingSecurityLevel::importKey");
        security_level_manager::notify_operation_performed(self.security_level);
        let (latency, result) =
    ...
    fn importWrappedKey(
        ...
        let ctx = Some(require_ko_bing_ctx(
            ctx,
            "IKoBingSecurityLevel::importWrappedKey",
        )?);
        let _wp = self.watch("IKoBingSecurityLevel::importWrappedKey");
        security_level_manager::notify_operation_performed(self.security_level);
        let (latency, result) = crate::timed_call!(self.import_wrapped_key(
            ctx,
            key,
    ...
    fn convertStorageKeyToEphemeral(
        ...
        let ctx = Some(require_ko_bing_ctx(
            ctx,
            "IKoBingSecurityLevel::convertStorageKeyToEphemeral",
        )?);
        let _wp = self.watch("IKoBingSecurityLevel::convertStorageKeyToEphemeral");
        security_level_manager::notify_operation_performed(self.security_level);
    ...
    fn deleteKey(&self, ctx: Option<&CallerInfo>, key: &KeyDescriptor) -> Result<(), Status> {
        let ctx = Some(require_ko_bing_ctx(ctx, "IKoBingSecurityLevel::deleteKey")?);
        let _wp = self.watch("IKoBingSecurityLevel::deleteKey");
        security_level_manager::notify_operation_performed(self.security_level);
