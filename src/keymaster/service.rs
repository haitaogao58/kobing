...
use crate::keymaster::permission::{
    check_grant_permission, check_key_permission, check_keystore_permission, require_ko_bing_ctx,
...
use crate::top::kobing::ko_bing::IKoBingKsService::IKoBingKsService;
use crate::top::kobing::ko_bing::IKoBingSecurityLevel::IKoBingSecurityLevel;
...
struct SecurityLevels {
    ...
    i_osec_level_by_uuid: HashMap<Uuid, Strong<dyn IKoBingSecurityLevel>>,
...
impl KeystoreService {
    ...
    fn get_ko_bing_security_level(
        &self,
        sec_level: SecurityLevel,
    ) -> Result<Strong<dyn IKoBingSecurityLevel>> {
        self.ensure_security_levels_current()?;
        let security_levels = self.security_levels.read().unwrap();
        if let Some(dev) = security_levels
            .uuid_by_sec_level
    ...
    fn is_ko_bing_grant(&self, ctx: Option<&CallerInfo>, grant: &KeyDescriptor) -> Result<bool> {
        ...
        match DB.with(|db| {
            ...
            Err(error) => match error.root_cause().downcast_ref::<Error>() {
                ...
                _ => Err(error).context(err!("KeystoreService::is_ko_bing_grant.")),
            },
        }
    }
...
#[allow(non_snake_case)]
impl IKoBingKsService for KeystoreService {
    fn getSecurityLevel(
        &self,
        security_level: SecurityLevel,
    ) -> Result<Strong<dyn IKeystoreSecurityLevel>, Status> {
        let _wp = wd::watch_millis_with("IKoBingKsService::getSecurityLevel", 500, security_level);
        self.get_security_level(security_level)
            .map_err(into_logged_binder)
    }

    fn getKoBingSecurityLevel(
        &self,
        security_level: SecurityLevel,
    ) -> Result<Strong<dyn IKoBingSecurityLevel>, Status> {
        let _wp = wd::watch_millis_with("IKoBingKsService::getSecurityLevel", 500, security_level);
        self.get_ko_bing_security_level(security_level)
            .map_err(into_logged_binder)
    }
    ...
    fn getKeyEntry(
        ...
        let _wp = wd::watch("IKoBingKsService::get_key_entry");
        let ctx = Some(require_ko_bing_ctx(ctx, "IKoBingKsService::getKeyEntry")?);
    ...
    fn updateSubcomponent(
        ...
        let _wp = wd::watch("IKoBingKsService::updateSubcomponent");
        let ctx = Some(require_ko_bing_ctx(
            ctx,
            "IKoBingKsService::updateSubcomponent",
        )?);
    ...
    fn listEntries(
        ...
        let _wp = wd::watch("IKoBingKsService::listEntries");
        let ctx = Some(require_ko_bing_ctx(ctx, "IKoBingKsService::listEntries")?);
        self.list_entries(ctx, domain, namespace)
            .map_err(into_logged_binder)
    }

    fn deleteKey(&self, ctx: Option<&CallerInfo>, key: &KeyDescriptor) -> Result<(), Status> {
        let _wp = wd::watch("IKoBingKsService::deleteKey");
        let ctx = Some(require_ko_bing_ctx(ctx, "IKoBingKsService::deleteKey")?);
    ...
    fn grant(
        ...
        let _wp = wd::watch("IKoBingKsService::grant");
        let ctx = Some(require_ko_bing_ctx(ctx, "IKoBingKsService::grant")?);
    ...
    fn ungrant(
        ...
        let _wp = wd::watch("IKoBingKsService::ungrant");
        let ctx = Some(require_ko_bing_ctx(ctx, "IKoBingKsService::ungrant")?);
    ...
    fn getNumberOfEntries(
        ...
        let _wp = wd::watch("IKoBingKsService::getNumberOfEntries");
        let ctx = Some(require_ko_bing_ctx(
            ctx,
            "IKoBingKsService::getNumberOfEntries",
        )?);
    ...
    fn listEntriesBatched(
        ...
        let _wp = wd::watch("IKoBingKsService::listEntriesBatched");
        let ctx = Some(require_ko_bing_ctx(
            ctx,
            "IKoBingKsService::listEntriesBatched",
        )?);
    ...
    fn getSupplementaryAttestationInfo(&self, tag: Tag) -> Result<Vec<u8>, Status> {
        let _wp = wd::watch("IKoBingKsService::getSupplementaryAttestationInfo");
        self.get_supplementary_attestation_info(tag)
            .map_err(into_logged_binder)
    }
    ...
    fn updateEcKeybox(
        ...
        let ctx = require_ko_bing_ctx(ctx, "IKoBingKsService::updateEcKeybox")?;
    ...
    fn updateRsaKeybox(
        ...
        let ctx = require_ko_bing_ctx(ctx, "IKoBingKsService::updateRsaKeybox")?;
        self.enforce_keybox_admin(ctx).map_err(into_logged_binder)?;
        let chain: Vec<kmr_wire::keymint::Certificate> = chain
            .iter()
            .map(|c| kmr_wire::keymint::Certificate {
    ...
    fn isKoBingGrant(
        ...
        let _wp = wd::watch("IKoBingKsService::isKoBingGrant");
        let ctx = Some(require_ko_bing_ctx(ctx, "IKoBingKsService::isKoBingGrant")?);
        self.is_ko_bing_grant(ctx, grant)
            .map_err(into_logged_binder)
    }
}
