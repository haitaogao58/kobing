...
use crate::{
    ...
    top::kobing::ko_bing::IKoBingKsService::BnKoBingKsService,
};
...
fn repair_ko_bing_data_files() {
    let entries = match std::fs::read_dir(root_path!("data")) {
        Ok(entries) => entries,
        Err(e) => {
            storage_warn(format!("Failed to list KOBING data directory: {e:?}"));
            return;
        }
    };
    ...
    for entry in entries {
        let entry = match entry {
            Ok(entry) => entry,
            Err(e) => {
                storage_warn(format!("Failed to read KOBING data directory entry: {e:?}"));
                continue;
            }
        };
        let path = entry.path();
        let file_type = match entry.file_type() {
            ...
            Err(e) => {
                storage_warn(format!("Failed to stat KOBING data file {path:?}: {e:?}"));
                continue;
            }
        };
        ...
        if let Err(e) = std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o600)) {
            storage_warn(format!("Failed to chmod KOBING data file {path:?}: {e:?}"));
        }
        let Some(path) = path.to_str() else {
            storage_warn(format!("Skipping non-UTF8 KOBING data file path {path:?}"));
            continue;
        };
        if let Err(e) = chown_path(path, KEYSTORE_UID, KEYSTORE_GID) {
            storage_warn(format!("Failed to chown KOBING data file {path}: {e:?}"));
        }
    }
}
...
fn prepare_android_storage() {
    for dir in [root_path!(), root_path!("data"), root_path!("logs")] {
        if let Err(e) = std::fs::create_dir_all(dir) {
            storage_warn(format!("Failed to create KOBING directory {dir}: {e:?}"));
            continue;
        }

        if let Err(e) = std::fs::set_permissions(dir, std::fs::Permissions::from_mode(0o770)) {
            storage_warn(format!("Failed to chmod KOBING directory {dir}: {e:?}"));
        }

        if let Err(e) = chown_path(dir, KEYSTORE_UID, KEYSTORE_GID) {
            storage_warn(format!("Failed to chown KOBING directory {dir}: {e:?}"));
        }
    }
    ...
    for file in [
        ...
        match std::fs::remove_file(file) {
            ...
            Err(e) => storage_warn(format!(
                "Failed to remove legacy KOBING lock file {file}: {e:?}"
            )),
        }
    }
    ...
    for file in [
        ...
        if let Err(e) = std::fs::set_permissions(file, std::fs::Permissions::from_mode(mode)) {
            storage_warn(format!("Failed to chmod KOBING file {file}: {e:?}"));
        }

        if let Err(e) = chown_path(file, KEYSTORE_UID, KEYSTORE_GID) {
            storage_warn(format!("Failed to chown KOBING file {file}: {e:?}"));
        }
    }
}
...
fn create_rpc_server() -> Result<Arc<RpcServer>> {
    set_sockcreate_con(RPC_SOCKET_CONTEXT)
        .context("failed to set KOBING RPC socket SELinux context")?;
    let server = RpcServer::setup_unix_server(rpc::SOCKET);
    let clear_result =
        clear_sockcreate_con().context("failed to clear KOBING RPC socket SELinux context");
    let server = server.context("failed to bind KOBING RPC socket")?;
    clear_result?;
    server.set_android13plus(rpc::WIRE_MAX_VERSION);
    std::fs::set_permissions(rpc::SOCKET, std::fs::Permissions::from_mode(0o660))
        .context("failed to chmod KOBING RPC socket")?;
    ...
    server.set_authorizer(|peer| {
        ...
        if !allowed {
            warn!("rejected KOBING RPC peer {peer}");
        }
        allowed
    });
...
fn run() -> Result<()> {
    ...
    let dev = KeystoreService::new_native_binder().context("failed to create ko_bing service")?;

    info!("adding KOBING service to RPC server");
    let service = BnKoBingKsService::new_binder_with_features(dev, consts::sid_features());
    server
        .add_service(rpc::SERVICE, service.as_binder())
        .context("failed to add KOBING RPC service")?;

    info!("creating KOBING authorization service");
    let auth = AuthorizationManager::new_ko_bing_binder()
        .context("failed to create KOBING authorization service")?;
    info!("adding KOBING authorization service to RPC server");
    server
        .add_service(rpc::AUTHORIZATION_SERVICE, auth.as_binder())
        .context("failed to add KOBING authorization RPC service")?;

    info!("creating KOBING maintenance service");
    let maintenance = MaintenanceManager::new_ko_bing_binder()
        .context("failed to create KOBING maintenance service")?;
    info!("adding KOBING maintenance service to RPC server");
    server
        .add_service(rpc::MAINTENANCE_SERVICE, maintenance.as_binder())
        .context("failed to add KOBING maintenance RPC service")?;

    info!("creating KOBING metrics service");
    let metrics =
        Metrics::new_native_binder().context("failed to create KOBING metrics service")?;
    info!("adding KOBING metrics service to RPC server");
    server
        .add_service(rpc::METRICS_SERVICE, metrics.as_binder())
        .context("failed to add KOBING metrics RPC service")?;

    info!("serving KOBING RPC socket={}", rpc::SOCKET);
    server.run().context("KOBING RPC server stopped")?;
    Ok(())
}
