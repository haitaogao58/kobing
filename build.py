#!/usr/bin/env python3
"""
Build script for KoBing Android targets.
"""
...
def delete_old_zips(release: bool, selected_abis: list[str]) -> None:
    ...
    for abi in selected_abis:
        pattern = TARGET_ROOT / f"KoBing-{build_type}-{abi}-*.zip"
...
def create_zip_package(
    *,
    ...
    build_type = "release" if release else "debug"
    zip_path = TARGET_ROOT / f"KoBing-{build_type}-{abi}-{version}-{git_hash}.zip"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build KoBing Magisk packages for Android")
    ...
    print(f"Building KoBing version {version} (commit {git_count}, hash {git_hash})")
