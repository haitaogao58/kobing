#!/usr/bin/env python3
"""
Build, deploy, and hot-restart KOBING artifacts.

The script never reboots the device. It updates /data/adb/ko_bing/keymint and
/data/adb/ko_bing/inject, then requests the module daemons to restart through the
...
def latest_full_package(abi: str, release: bool) -> Path:
    ...
    candidates = list(TARGET_ROOT.glob(f"KoBing-{build_type}-{abi}-*.zip"))
    if not candidates:
...
def deploy_binaries(
    ...
    suffix = os.getpid()
    remote_keymint_stage = remote_path(staging_dir, f"ko_bing.keymint.{suffix}.new")
    remote_inject_stage = remote_path(staging_dir, f"ko_bing.inject.{suffix}.new")
    ...
    command = (
        ...
        f"rm -f /data/misc/keystore/ko_bing/rpc.sock; "
...
def install_full_package(
    ...
    remote_package = remote_path(staging_dir, f"ko_bing.module.{os.getpid()}.zip")
...
def trigger_restart(serial: str | None, restart: str) -> None:
    ...
    adb_shell_root(serial, command)
    print_status(f"Requested KOBING restart target: {restart}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build, deploy, hot-restart, or fully install KOBING artifacts",
