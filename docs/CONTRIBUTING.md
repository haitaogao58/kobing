...
- Do not provide key or descriptor continuity between System and KOBING. Pass old,
  ...
  unchanged. An KOBING business error for such a descriptor is authoritative.
- Return KOBING business errors unchanged. Reachable transport, boundary, or
  injector failures that are not KOBING-unavailable must not fall back to a
...
- Only confirmed KOBING-unavailable failures may preserve the original System
...
2. Your Contribution, as part of this project, is subject to and complies with
   ...
   and the [Ko Bing License](../LICENSE-2). The two licenses apply together;
