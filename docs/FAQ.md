...
This page is for people who use KoBing (KOBING) without needing to
...
information. KOBING provides an alternative version of that service for the apps
you select.

KOBING only covers this part of Android. It is not a general root-hiding tool, a
Play Integrity module, or a repair tool for damaged security hardware.

### Will KOBING make every banking app, payment app, or detector work?

No. Apps may check root, installed modules, system properties, certificates,
hooks, their own files, and many other signals that KOBING does not control. A
...
If an app shows exactly the same warning with and without KOBING, that warning
probably comes from something KOBING does not control. If the message or behavior
...
### Does KOBING add StrongBox to a phone that does not have it?

No. StrongBox is hardware. KOBING cannot create it on a device that does not have
it. Apps that require real StrongBox may still reject such a device.

### Can I use KOBING with other root or KeyStore modules?

Use only one root implementation. KernelSU and Magisk are the supported ways
to install KOBING; recovery installation is not supported.

Do not run KOBING together with another module that replaces Android's KeyStore
...
### How should I update KOBING?

Install the current release over the existing installation in the root manager,
then reboot. The update keeps the active settings, keybox, and stored KOBING keys.
...
### What happens if I disable or uninstall KOBING?

Disable or remove it in the root manager and reboot. Keys created through KOBING
are not converted into normal System keys, so some selected apps may ask you
to sign in or register again after KOBING is disabled.

KOBING's persistent data is stored outside the module folder and may remain after
...
after `id=`. Add only apps that you actually want KOBING to handle.
...
KOBING is designed for selected apps, and there is no official one-click
"all apps" mode. Keep the filter enabled and add only the apps that need KOBING.
...
values can make existing KOBING keys unusable.
...
version is not applied while KOBING is already running; the affected component
...
a broken `injector.toml` disables KOBING request routing.
...
startup, KOBING creates a new default file. Do not delete `config.toml` as a reset
method: its newly generated `[crypto]` values may make existing KOBING keys
unusable.

### Does an update overwrite my settings or keybox?

No. KOBING preserves the active settings and keybox files during an update. The
...
KOBING requires one complete RSA entry and one complete EC entry. For both
...
tools accept damaged or incomplete XML that KOBING correctly rejects.
...
KOBING normally reloads it automatically. Check `keymint.log` afterward. If the
file is missing or invalid, KOBING records the reason and restores the bundled
...
### What should I check first when KOBING appears not to work?
...
test app and an KOBING routing result.
...
damaged. They can appear when the test app was not routed to KOBING, injection
failed, or the active keybox was missing, invalid, expired, or revoked.

Confirm the app's package, routing log, and keybox log from the same test. KOBING
...
still occurs after a fresh app setup on a current KOBING build, report it with
matching logs.

### Why does an app still report root or show a security warning?

KOBING does not hide root. A warning can come from an app's own root detection,
...
or "secure" label as the same result. Use KOBING's routing and keymint logs to
decide whether KOBING handled the request.
...
as `KEY_USER_NOT_AUTHENTICATED` does not always mean KOBING is broken.
...
Look for a real user-visible failure at the same time. Do not erase all KOBING
...
keys, and rejected invalid requests may be logged even when KOBING is working as
...
or module-disable method to disable KOBING. Do not keep testing a broad
...
level requires a keymint restart. KOBING rotates the logs; all four files together
are limited to about 16 MB in normal operation.

For a report, note the exact reproduction time and also collect
`logcat -d -s KoBing` from the same attempt.
...
- a same-build comparison with and without KOBING routing when the problem affects
  one app.

Screenshots alone are rarely enough. Do not upload the whole KOBING data folder.
...
[Ko Bing License](../LICENSE-2) apply together. The additional license
prohibits commercial use and contains other conditions. Read both before using
or distributing KOBING.
