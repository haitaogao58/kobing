...
- A malformed `injector.toml` present when the injector starts leaves KOBING
...
- If either file is missing when its component starts, KOBING creates a new file
...
`2`. It is not an Android version or an KOBING release number. Do not increment it;
...
cannot verify biometric authentication tokens correctly. When `true`, KOBING
...
as 64 hexadecimal characters using `0-9` and `a-f`. KOBING generates these values
when it creates a new configuration.

Keep all four generated seed and nonce fields present and stable, and back them
up privately with the KOBING data. Changing or removing any of them requires a
...
If `shared_secret_seed` or `shared_secret_nonce` is missing, KOBING generates a
...
This seed is used to derive key material that protects KOBING key blobs. If it
changes, KOBING may no longer be able to open keys created with the previous
value. It must be present and must remain unchanged.

#### `kak_seed`

This seed is used for KOBING's key-agreement protection. It belongs to the same
...
When it is absent, KOBING derives the required key through
...
existing runtime property, but KOBING never creates or deletes it. `"auto"` never
...
merely because parsing later fails. KOBING does not write the vendor property.
...
If the property is absent, KOBING reads the same property from the active boot
...
fails, KOBING uses this fallback order: nonempty runtime
...
With the recommended value `false`, KOBING tries to fill only empty `imei`,
`imei2`, and `meid` fields from the device's telephony services and property
fallbacks. A configured non-empty value is preserved, and KOBING attempts to write
each successfully discovered value back to the active `config.toml`.

With `true`, KOBING skips telephony discovery and uses the three configured fields
...
It is not an Android version or an KOBING release number. Live reload rejects
...
`[intercept]` settings decide each new request. `false` stops new KOBING routing so
normal requests continue to System. Keep it `true` for normal KOBING use.
...
With the filter enabled, KOBING evaluates a caller in this order:
...
Disabling the filter can route Android services and unrelated apps to KOBING and
...
allowed by the filter, `true` routes that operation to KOBING and `false` leaves
...
Controls deletion of a named key. The selected backend is authoritative; KOBING
