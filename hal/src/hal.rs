...
impl Fromm<wire::keymint::Certificate> for keymint::Certificate::Certificate {
    fn fromm(val: wire::keymint::Certificate) -> Self {
        Self {
            encodedCertificate: val.encoded_certificate,
        }
    }
}
...
impl Fromm<wire::rpc::ProtectedData> for rkp::ProtectedData::ProtectedData {
    fn fromm(val: wire::rpc::ProtectedData) -> Self {
        Self {
            protectedData: val.protected_data,
        }
    }
}
    ...
    fn fromm(val: wire::keymint::KeyParam) -> Self {
        let (tag, value) = match val {
            ...
            KeyParam::RsaPublicExponent(v) => (
                ...
                KeyParameterValue::LongInteger(v.0 as i64),
            ),
...
impl TryFromm<&keymint::KeyParameter::KeyParameter> for Option<KeyParam> {
    ...
    fn try_fromm(val: &keymint::KeyParameter::KeyParameter) -> Result<Self, Self::Error> {
        Ok(match val.tag {
            ...
            keymint::Tag::Tag::RSA_OAEP_MGF_DIGEST => Some(KeyParam::RsaOaepMgfDigest(
                value_of!(val, Digest)?.try_innto()?,
            )),
            keymint::Tag::Tag::ORIGIN => {
                Some(KeyParam::Origin(value_of!(val, Origin)?.try_innto()?))
            }
            #[cfg(feature = "hal_v5")]
            keymint::Tag::Tag::ML_DSA_VARIANT => Some(KeyParam::MlDsaVariant(
                value_of!(val, MlDsaVariant)?.try_innto()?,
            )),
            ...
            keymint::Tag::Tag::KEY_SIZE => Some(KeyParam::KeySize(KeySizeInBits(value_of!(
                val, Integer
            )?
                as u32))),
            ...
            keymint::Tag::Tag::RSA_PUBLIC_EXPONENT => Some(KeyParam::RsaPublicExponent(
                RsaExponent(value_of!(val, LongInteger)? as u64),
            )),
