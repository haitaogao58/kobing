...
use crate::top::kobing::ko_bing::IKoBingAuthorizationService::IKoBingAuthorizationService;
use crate::top::kobing::ko_bing::IKoBingKsService::IKoBingKsService;
use crate::top::kobing::ko_bing::IKoBingMaintenanceService::IKoBingMaintenanceService;
...
pub fn install_direct_rpc_session() -> Result<()> {
    ...
    let cache = connect_rpc_session("failed to connect KOBING RPC socket")?;
    let old = {
    ...
    loop {
        match connect_rpc_session_once(connect_context) {
            ...
            Err(error) if start.elapsed() >= RPC_READY_TIMEOUT => {
                return Err(error).context("KOBING RPC server did not become ready in time");
            }
...
pub fn get_ko_bing() -> Result<Strong<dyn IKoBingKsService>> {
    get_rpc_binder(rpc::SERVICE, "failed to connect to ko_bing service", false)
}

pub fn with_ko_bing_retry<T, F>(mut f: F) -> Result<T>
where
    F: FnMut(&Strong<dyn IKoBingKsService>) -> Result<T>,
{
    with_binder_retry(
        "ko_bing",
        get_ko_bing,
...
pub fn with_ko_bing_once<T, F>(f: F) -> Result<T>
where
    F: FnOnce(&Strong<dyn IKoBingKsService>) -> Result<T>,
{
    with_binder_once(
        || get_rpc_binder(rpc::SERVICE, "failed to connect to ko_bing service", true),
        |client| {
            clear_rpc_cache_if(rpc::SERVICE, &client.as_binder());
        },
        is_rpc_cache_invalidating_error,
...
fn get_ko_bing_authorization_fresh() -> Result<Strong<dyn IKoBingAuthorizationService>> {
    get_rpc_binder(
...
pub fn with_ko_bing_authorization_once<T, F>(f: F) -> Result<T>
where
    F: FnOnce(&Strong<dyn IKoBingAuthorizationService>) -> Result<T>,
{
    with_binder_once(
        get_ko_bing_authorization_fresh,
        |client| {
...
fn get_ko_bing_maintenance_fresh() -> Result<Strong<dyn IKoBingMaintenanceService>> {
    get_rpc_binder(
...
pub fn with_ko_bing_maintenance_once<T, F>(f: F) -> Result<T>
where
    F: FnOnce(&Strong<dyn IKoBingMaintenanceService>) -> Result<T>,
{
    with_binder_once(
        get_ko_bing_maintenance_fresh,
        |client| {
