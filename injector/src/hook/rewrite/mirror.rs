...
pub(super) fn reserve_mirror_update(
    ...
    if state.authorization.poisoned || state.maintenance.poisoned {
        anyhow::bail!("KOBING mirror recovery is poisoned by a non-retryable failure");
    }
...
impl Drop for MirrorUpdateReservation {
    fn drop(&mut self) {
        ...
        if self.failure_policy == MirrorFailurePolicy::FailClosed {
            warn!(
                "event=mirror {} update lost its System reply; KOBING routes are fail-closed until process restart",
                self.kind.label()
            );
        } else {
            warn!(
                "event=mirror {} best-effort update lost its System reply; dropping the mirror without blocking KOBING routes",
                self.kind.label()
            );
        }
        wake_mirror_recovery_worker();
...
pub(super) fn ensure_mirror_state_recovered() -> anyhow::Result<()> {
    ...
    if state.authorization.draining || state.maintenance.draining {
        anyhow::bail!("KOBING mirror recovery is busy");
    }
    ...
    drop(state);
    wake_mirror_recovery_worker();
    anyhow::bail!("KOBING mirror recovery is pending")
}
    ...
    loop {
        ...
        match result {
            ...
            Err(error) => {
                ...
                drop(state);
                warn!(
                    "event=mirror replay sequence={} domain={} method={} failed: {:#}; preserving the in-memory event and blocking KOBING routes",
                    pending.sequence,
                    pending.kind.label(),
                    method,
                    error
...
pub(in crate::hook) fn start_mirror_recovery_worker() -> Result<(), String> {
    let worker = MIRROR_RECOVERY_WORKER.get_or_init(|| {
        ...
        std::thread::Builder::new()
            .name("ko_bing-mirror-recovery".to_string())
...
pub(super) unsafe fn build_authorization_reply_mirror(
    ...
    if !status.is_ok() {
        ...
        debug!(
            "event=mirror domain=authorization system {:?} failed with {}; skipping KOBING mirror",
            method, status
        );
        return Ok(None);
    }
    ...
    let Some(reservation) = call.mirror_update.take() else {
        if failure_policy == MirrorFailurePolicy::BestEffort {
            warn!(
                "event=mirror domain=authorization no queue reservation for best-effort {:?} uid={} pid={}; preserving System success without an KOBING mirror",
    ...
    match reservation.enqueue(event) {
        ...
        Err(error) if failure_policy == MirrorFailurePolicy::BestEffort => {
            warn!(
                "event=mirror domain=authorization failed to queue best-effort {:?} for KOBING for uid={} pid={}: {:#}; preserving System success",
        ...
        Err(error) => {
            warn!(
                "event=mirror domain=authorization failed to queue {:?} for KOBING for uid={} pid={}: {:#}",
                method, call.caller.uid, call.caller.pid, error
            );
...
fn execute_authorization_mirror(
    ...
    match request {
        ParsedAuthorizationRequest::AddAuthToken { auth_token } => {
            ipc::with_ko_bing_authorization_once(|auth| {
...
pub(super) unsafe fn build_maintenance_reply_mirror(
    ...
    if !status.is_ok() {
        ...
        debug!(
            "event=mirror domain=maintenance system {:?} failed with {}; skipping KOBING mirror",
            method, status
        );
        return Ok(None);
    }
    if !maintenance_requires_mirror(&call.request) {
        debug!(
            "event=mirror domain=maintenance {:?} is read-only or has no KOBING mirror endpoint; preserving system reply",
            method
        );
        return Ok(None);
    }
    ...
    match reservation.enqueue(event) {
        ...
        Err(error) => {
            warn!(
                "event=mirror domain=maintenance failed to queue {:?} for KOBING for uid={} pid={}: {:#}",
                method, call.caller.uid, call.caller.pid, error
            );
            Ok(Some(synthetic_fallback_reply()))
        }
...
fn execute_maintenance_mirror(
    ...
    match request {
        ParsedMaintenanceRequest::OnUserAdded { user_id } => {
            ipc::with_ko_bing_maintenance_once(|maintenance| {
