...
#[test]
fn ko_bing_grant_blocker_preserves_inbound_binder_buffer() {
...
#[test]
fn grant_and_ungrant_preserve_system_only_when_ko_bing_unavailable() {
    ...
    let result = precompute_ko_bing_grant_service_reply_with(
        &request,
        &caller,
    ...
    assert!(matches!(result, KoBingServicePrecompute::PreserveSystem));
...
#[test]
fn grant_precompute_returns_reachable_ko_bing_business_error() {
    let request = ParsedServiceRequest::Grant {
        key: sample_key_descriptor(),
        grantee_uid: 12345,
        access_vector: 7,
    ...
    let KoBingServicePrecompute::Reply(PrecomputedServiceReply::Error(status)) = result else {
        panic!("reachable KOBING business error should be precomputed as an authoritative status");
    };
    assert_eq!(
...
fn service_route_respects_intercept_configuration() {
    ...
    for request in sample_service_requests() {
        assert_eq!(
            ...
            RouteTarget::KoBing,
            "{:?} should use the KOBING backend when the method is enabled",
            request.method()
        );
    }
...
#[test]
fn service_route_uses_ko_bing_for_key_id() {
...
fn unconfirmed_grant_readback_stays_rejected() {
    ...
    assert!(!should_allow_ko_bing_grant_service_request_with_probe(
...
fn soft_rejected_grant_probe_error_is_fail_closed() {
    ...
    let error = should_allow_ko_bing_grant_service_request_with_probe(
        &ParsedServiceRequest::GetKeyEntry { key: grant },
        &decision,
        &caller,
...
fn soft_rejected_grant_positive_probe_works_for_service_and_security_level() {
    ...
    assert!(
        should_allow_ko_bing_grant_security_level_request_with_probe(
...
#[test]
fn denylisted_grant_readback_does_not_probe_ko_bing() {
    let _guard = route_state_test_guard();
    let grant = KeyDescriptor {
        domain: Domain::GRANT,
        nspace: 123,
    ...
    assert!(!should_allow_ko_bing_grant_service_request_with_probe(
        ...
        |_, _| -> anyhow::Result<bool> {
            panic!("denylisted callers must not probe KOBING grants")
        },
    )
