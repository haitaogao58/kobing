...
pub(super) enum KoBingServicePrecompute {
    Reply(PrecomputedServiceReply),
    PreserveSystem,
}

pub(super) enum KoBingMigratePrecompute {
    ...
    match result {
        ...
        Ok(None) => {
            let observed = match &pending {
                PendingCall::Service(call) if call.route == RouteTarget::KoBing => {
                    observe_system_service_reply(tr, call)
                }
                PendingCall::SecurityLevel(call) if call.route == RouteTarget::KoBing => {
        ...
        Err(error) => {
            if pending_preserves_system_on_rewrite_failure(&pending) {
                ...
                warn!(
                    "event=reply failed to rewrite authoritative KOBING reply: {:#}; returning SYSTEM_ERROR",
                    error
                );
...
fn pending_preserves_system_on_rewrite_failure(pending: &PendingCall) -> bool {
    match pending {
        ...
        PendingCall::Maintenance(call) => call.route != RouteTarget::KoBing,
        PendingCall::PrecomputedMaintenance(_, _) => false,
        PendingCall::Service(call) => call.route != RouteTarget::KoBing,
        PendingCall::PrecomputedService(_, _) => false,
        PendingCall::SecurityLevel(call) => call.route != RouteTarget::KoBing,
