...
fn route_for_service_request(
    ...
    if identify::is_ko_bing_service_route_enabled(request.method(), intercept) {
        RouteTarget::KoBing
    } else {
        RouteTarget::System
    }
...
pub(in crate::hook) unsafe fn handle_br_transaction(
    ...
    if request_interface == identify::KEYSTORE_AUTHORIZATION_INTERFACE {
        ...
        info!(
            "event=decision command={} authorization_method={:?} code=0x{:x} uid={} pid={} sid='{}'; mirroring auth state to KOBING after system success",
            command_name,
            method,
            tr.code,
            caller.uid,
        ...
        if requires_mirror && !expects_reply {
            if failure_policy == MirrorFailurePolicy::BestEffort {
                warn!(
                    "event=mirror cannot confirm one-way best-effort authorization {:?} for uid={} pid={}; preserving System mutation without an KOBING mirror",
        ...
        if requires_mirror {
            match reserve_mirror_update(MirrorStateKind::Authorization, failure_policy) {
                ...
                Err(error) if failure_policy == MirrorFailurePolicy::BestEffort => {
                    warn!(
                        "event=mirror failed to reserve in-memory best-effort authorization {:?} update for uid={} pid={}: {:#}; preserving System mutation without an KOBING mirror",
    ...
    if request_interface == identify::KEYSTORE_MAINTENANCE_INTERFACE {
        ...
        let mut precomputed = if pending.route == RouteTarget::KoBing
            && matches!(
                &pending.request,
                ParsedMaintenanceRequest::MigrateKeyNamespace { .. }
            ) {
            match precompute_ko_bing_migrate_reply(&pending) {
                KoBingMigratePrecompute::Reply(reply) => {
                    block_system_request(tr);
                    Some(reply)
                }
        ...
        let route_note = if matches!(
            ...
            "mirroring maintenance state to KOBING after system success"
        };
        info!(
    ...
    if request_interface == identify::KEYSTORE_SERVICE_INTERFACE {
        ...
        let allow_ko_bing_grant = match should_allow_ko_bing_grant_service_request_with_probe(
            &request,
            &decision,
            &caller,
            probe_ko_bing_grant,
            ...
            Err(error) => {
                warn!(
                    "event=decision KOBING grant ownership probe failed for uid={} pid={}: {:#}; returning SYSTEM_ERROR without executing System",
        ...
        let mut route = if allow_ko_bing_grant {
            RouteTarget::KoBing
        } else {
            route_for_service_request(&request, &cfg.intercept)
        };
        if !decision.allowed && !allow_ko_bing_grant {
            info!(
        ...
        if allow_ko_bing_grant {
            info!(
                "event=decision command={} service_method={:?} code=0x{:x} uid={} pid={} sid='{}' packages={:?} allowed=true reason={:?} ko_bing_grant=true",
                command_name,
                method,
                original_code,
                caller.uid,
        ...
        let precomputed_service_reply = if route == RouteTarget::KoBing
            && matches!(
                method,
            ...
            match precompute_ko_bing_service_mutator_reply(&request, &caller) {
                KoBingServicePrecompute::Reply(reply) => {
                    block_system_request(tr);
                    Some(reply)
                }
                KoBingServicePrecompute::PreserveSystem => {
                    info!(
                        "event=route method={:?} uid={} pid={} route={:?} ko_bing_unavailable=true; preserving original system request",
        ...
        if !expects_reply {
            if let Some(reply) = precomputed_service_reply.as_ref() {
                if let Err(error) = build_precomputed_service_reply(reply) {
                    warn!(
                        "event=route failed to build one-way KOBING service {:?} reply for uid={} pid={}: {:#}",
                        method, caller_uid, pending.caller.pid, error
                    );
                }
                return request_rewritten;
    ...
    if request_interface == identify::KEYSTORE_SECURITY_LEVEL_INTERFACE {
        ...
        let method = request.method();
        let allow_unknown_ko_bing_route =
            match should_allow_ko_bing_grant_security_level_request_with_probe(
                &request,
                &decision,
                &caller,
                probe_ko_bing_grant,
        ...
        if !decision.allowed && !allow_unknown_ko_bing_route {
            info!(
        ...
        info!(
            "event=decision command={} security_level_method={:?} code=0x{:x} uid={} pid={} sid='{}' packages={:?} allowed={} reason={:?} target=ptr:0x{:x}/cookie:0x{:x} security_level={:?} scoop_enabled={} ko_bing_derived_route={}",
            command_name,
            method,
            tr.code,
            caller.uid,
        ...
        if expects_reply && pending.route == RouteTarget::KoBing {
            let reply = match build_ko_bing_security_level_reply(&pending, true) {
                Ok(Some(reply)) => reply,
                Ok(None) => {
                    info!(
                        "event=route security-level {:?} KOBING unavailable for uid={} pid={}; preserving original system request",
                        method, caller_uid, pending.caller.pid
                    );
                    pending.route = RouteTarget::System;
                ...
                Err(error) => {
                    warn!(
                        "event=route failed to precompute authoritative KOBING security-level {:?} reply for uid={} pid={}: {:#}; returning SYSTEM_ERROR without executing System",
                        method, caller_uid, pending.caller.pid, error
                    );
        ...
        {
            return handle_ko_bing_one_way_security_level_request(tr, &pending);
        }
        if expects_reply {
            replace_top_pending(connection, PendingCall::SecurityLevel(pending));
        }
    ...
    if request_interface == identify::KEYSTORE_OPERATION_INTERFACE {
        ...
        if !expects_reply
            && operation_target.route == RouteTarget::KoBing
            && can_execute_one_way(SyntheticTargetKind::Operation, tr.code)
        {
            return handle_ko_bing_one_way_operation_request(tr, &pending);
        }
        if expects_reply {
...
unsafe fn handle_ko_bing_one_way_security_level_request(
    tr: &mut binder_transaction_data,
    ...
    match build_security_level_reply_rewrite(tr, pending) {
        ...
        Err(error) => {
            warn!(
                "event=route failed to execute one-way KOBING security-level {:?} for uid={} pid={}: {:#}; consuming original system request",
...
unsafe fn handle_ko_bing_one_way_operation_request(
    tr: &mut binder_transaction_data,
    pending: &PendingOperationCall,
) -> bool {
    if let Err(error) = build_operation_reply_rewrite(pending) {
        warn!(
            "event=route failed to execute one-way KOBING operation {:?} for uid={} pid={}: {:#}; consuming original system request",
            pending.request.method(), pending.caller.uid, pending.caller.pid, error
        );
    }
    block_system_request(tr);
