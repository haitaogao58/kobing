...
impl<'a> Password<'a> {
    ...
    /// Reproduce the accidental KDF used by older KoBing builds.
    #[doc(hidden)]
    pub fn derive_key_ko_bing_legacy(&self, salt: &[u8], out_len: usize) -> Result<ZVec, Error> {
        let prk = ko_bing_legacy_kdf_extract(self.get_key(), salt)?;
        ko_bing_legacy_kdf_expand(out_len, &prk, &[])
    }
...
/// Reproduce the accidental PBKDF2-based extract used by older KoBing builds.
#[doc(hidden)]
pub fn ko_bing_legacy_kdf_extract(secret: &[u8], salt: &[u8]) -> Result<ZVec, Error> {
    let max_size: usize = EVP_MAX_MD_SIZE.try_into().unwrap();
    let mut buf = ZVec::new(max_size)?;
...
/// Reproduce the accidental PBKDF2-based expand used by older KoBing builds.
#[doc(hidden)]
pub fn ko_bing_legacy_kdf_expand(out_len: usize, prk: &[u8], info: &[u8]) -> Result<ZVec, Error> {
    let mut buf = ZVec::new(out_len)?;
